						...
						<c:when test="${!empty productList }">
							<c:forEach var="vo" items="${productList }" varStatus="status">
								<tr>
									...
									<td>
										<button type="button" value="${vo.productNum }" onclick="delete_go(this)" class="btn btn-sm btn-outline-danger font-weight-bold" style="margin:10px;">
											삭제
										</button>
									</td>
								</tr>
	...
	function delete_go(productNum) {
		Swal.fire({
			  ...
			  if (result.isConfirmed) {
				  var userPw = prompt("비밀번호를 입력해주십시오.");
				  $.ajax({
					  type:'POST',
					  url:'productDeleteConfirm.do',
					  data:{
						  "userPw": userPw, 
